...
class FanDirTemp(Observer):
    ...
    def _get_corrected_temp(self, sensors_state):
        sensors_state = sensors_state[settings.TEMP_SENSORS.MIXED] - 20

        if self._current_temp is None:
            self._current_temp = sensors_state

        temp_delta = abs(self._desired_l_temp - sensors_state)
        if self._desired_l_temp > sensors_state:
            self._current_temp = max(self._current_temp - temp_delta, min(settings.AC_TEMP_RANGE))
        elif self._desired_l_temp < sensors_state:
    ...
    def update(self, subject_type, subject):
        if subject_type == UART_TYPES.AC:
            print('Converted: {}'.format(convertGuiTemp(subject.l_temp.state)))
            self._desired_l_temp = convertGuiTemp(subject.l_temp.state)
            self._desired_r_temp = convertGuiTemp(subject.r_temp.state)
            self._ac_status = subject.ac_status
        if subject_type == CONTROLLER_NAMES.TEMP:
            if self._ac_status == AC_STATUS.OFF:
                self._current_temp = None
                return
            corrected_temp = self._get_corrected_temp(subject.sensors_state)
...
class ACFanPWM(BaseDutyRangePWM):
    def __init__(self):
        ...
        self._duty_range = settings.FAN_SPEED_RANGE
        self.init()
...
class ACFan(Observer):
    ...
    def update(self, subject_type, subject):
        if subject_type == UART_TYPES.AC:
            self._fan.state = subject.fan_speed.state
